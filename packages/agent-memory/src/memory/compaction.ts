...
import type { LLMMessage, LLMProvider } from "@inspect/llm";
...
export class ContextCompactor {
   ...
   */
  async compact(messages: LLMMessage[], llm: LLMProvider): Promise<CompactionResult> {
    ...
    const tokensAfter = this.estimateTokens([{ role: "system", content: summary }, ...toKeep]);
  ...
  private extractKeyInfo(messages: LLMMessage[]): string {
    ...
    for (const msg of messages) {
      ...
      if (content.includes("assert") || content.includes("verify") || content.includes("expect")) {
        const assertLine = content
          .split("\n")
          .find((l) => l.includes("assert") || l.includes("verify") || l.includes("expect"));
        if (assertLine) {
