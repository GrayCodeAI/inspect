...
describe("ElementMasking", () => {
  ...
  describe("maskRegions", () => {
    it("masks a region within the image", () => {
      ...
      const result = masking.maskRegions(img, [{ x: 2, y: 2, width: 3, height: 3 }]);
      // Masked pixels should differ from the original white
      const idx = (2 * 10 + 2) * 4;
      // At least the red channel should change from 255 to the mask color
      expect(
        result.data[idx] !== 255 || result.data[idx + 1] !== 255 || result.data[idx + 2] !== 255,
      ).toBe(true);
    });
    ...
    it("clamps regions to image bounds", () => {
      ...
      const result = masking.maskRegions(img, [{ x: 3, y: 3, width: 10, height: 10 }]);
    ...
    it("preserves image dimensions", () => {
      ...
      const result = masking.maskRegions(img, [{ x: 0, y: 0, width: 5, height: 5 }]);
