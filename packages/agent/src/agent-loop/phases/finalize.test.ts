...
import { finalizePhase, FinalizeInput } from "./finalize.js";

describe("finalize phase", () => {
  let baseInput: FinalizeInput;

  beforeEach(() => {
    baseInput = new FinalizeInput({
      stepNumber: 1,
      actionResults: [
  ...
  it("should record history entry", async () => {
    const result = await Effect.runPromise(finalizePhase(baseInput));
  ...
  it("should handle all successful actions", async () => {
    ...
    const result = await Effect.runPromise(finalizePhase(input));
