...
export class AuditTrail {
   ...
   */
  async log(entry: Omit<AuditEntry, "id" | "timestamp" | "hash" | "previousHash">): Promise<AuditEntry> {
    const id = generateId();
   ...
   */
  generateComplianceReport(standard: ComplianceReport["standard"], sessionId?: string): ComplianceReport {
  ...
  private toJunit(): string {
    ...
    const cases = this.entries.map((e) => {
      const status = e.result === "success" ? `<testcase name="${e.action}" time="${e.duration / 1000}"/>` : `<testcase name="${e.action}" time="${e.duration / 1000}"><failure message="${e.output}"/></testcase>`;
      return `    ${status}`;
    }).join("\n");
