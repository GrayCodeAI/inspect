...
Scenario: Add item to cart
  ...
  Then the item should appear in my cart`;
      ...
      assert.strictEqual(scenario.steps[2].text, "the item should appear in my cart");
    });
...
describe("StepRegistry", () => {
  ...
  it.effect(
    "matches a registered step",
    () =>
      Effect.gen(function* () {
        const registry = yield* StepRegistry;
        yield* registry.register("I am on the (.*) page", () => Effect.void);

        const definition = yield* registry.matchStep("I am on the login page");
        assert.ok(definition.pattern.test("I am on the login page"));
      }).pipe(Effect.provide(testLayer)) as Effect.Effect<void, never, never>,
  );

  it.effect(
    "fails with StepDefinitionNotFoundError for unregistered step",
    () =>
      Effect.gen(function* () {
        const registry = yield* StepRegistry;
        const result = yield* registry.matchStep("unknown step").pipe(Effect.flip);
...
describe("ScenarioRunner", () => {
  ...
  it.effect(
    "runs a scenario with all passing steps",
    () =>
      Effect.gen(function* () {
        const registry = yield* StepRegistry;
  ...
  it.effect(
    "tracks failed steps and collects errors",
    () =>
      Effect.gen(function* () {
        const registry = yield* StepRegistry;
        const runner = yield* ScenarioRunner;
  ...
  it.effect(
    "handles unregistered steps as passing (fallback)",
    () =>
      Effect.gen(function* () {
        const registry = yield* StepRegistry;
        const runner = yield* ScenarioRunner;
...
describe("Error classes", () => {
  it("GherkinParseError has correct display message", () => {
    ...
    assert.strictEqual(error.displayMessage, "Failed to parse test.feature at line 10");
  ...
  it("StepDefinitionNotFoundError has correct display message", () => {
    ...
    assert.strictEqual(error.displayMessage, 'No step definition found for "unknown step text"');
  });
