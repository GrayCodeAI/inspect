...
export class AIAnalysis {
  ...
  private buildPrompt(diff: VisualDiffResult, options?: AnalysisOptions): string {
    ...
    parts.push(`\nDiff stats: ${diff.mismatchPercentage.toFixed(2)}% mismatch.`);

    if (diff.diffRegions && diff.diffRegions.length > 0) {
      const region = diff.diffRegions[0];
      parts.push(
        `Changes are concentrated in region: x=${region.x}, y=${region.y}, ${region.width}x${region.height} pixels.`,
      );
    }
  ...
  private parseResponse(raw: string, diff: VisualDiffResult): AnalysisResult {
    try {
      ...
      return {
        ...
        shouldBlock: parsed.shouldBlock ?? diff.mismatchPercentage > 5,
  ...
  private fallbackAnalysis(diff: VisualDiffResult, rawResponse?: string): AnalysisResult {
    const percent = diff.mismatchPercentage;
    ...
    if (diff.diffRegions && diff.diffRegions.length > 0 && percent > 0) {
      const region = diff.diffRegions[0];
      changes.push({
        description: `Visual difference detected in a ${region.width}x${region.height} pixel region`,
        location: `Region at (${region.x}, ${region.y})`,
        category: "other",
    ...
    return {
      ...
      summary: `Pixel comparison found ${percent.toFixed(2)}% mismatch.${
        diff.diffRegions && diff.diffRegions.length > 0
          ? ` Changes concentrated at position (${diff.diffRegions[0].x}, ${diff.diffRegions[0].y}).`
          : ""
      }`,
      changes,
      shouldBlock,
