...
import { getPreset } from "../devices/presets.js";
...
export class DashboardOrchestrator {
  ...
  private async executeWithTracking(
    ...
    const promises = devices.map((device, i) =>
      this.executeOneRun(baseConfig, device, runIds[i]),
    );
  ...
  private toStepSnapshot(step: StepResult): DashboardStepSnapshot {
    return {
      ...
      toolCall: step.toolCalls.length > 0
  ...
  private computeFlakinessReport(): DashboardFlakinessReport {
    ...
    return {
      ...
      entries: report.scores.map((s: { testName: string; score: number; passRate: number; totalRuns: number; recommendation: "stable" | "flaky" | "broken" | "needs-investigation" }) => ({
        testName: s.testName,
        score: s.score,
        passRate: s.passRate,
  ...
  private inferActivityType(toolCall: string): AgentActivityType {
    ...
    if (toolCall.includes("verify") || toolCall.includes("assert") || toolCall.includes("console")) return "verifying";
    return "thinking";
  }
}
