...
import type { WorkflowBlock, WorkflowBlockResult } from "../../shared/types/workflow.js";
...
export async function executeProxyBlock(
  ...
  try {
    // ProxyServer requires @inspect/quality - dynamic import to avoid circular dependency
    // @ts-expect-error Optional dependency - not available during core build
    const qualityModule = await import("@inspect/quality").catch(() => null);
    if (!qualityModule) {
      return {
        blockId: block.id,
        status: "failed",
        error: "Proxy block requires @inspect/quality package to be installed",
        duration: Date.now() - start,
      };
    }
    const { ProxyServer } = qualityModule;
