...
export class HumanBehavior extends ServiceMap.Service<HumanBehavior>()(
  "@stealth/HumanBehavior",
  {
    make: Effect.gen(function* () {
      const generateMouseMovement = Effect.fn("HumanBehavior.generateMouseMovement")(
        function* (startX: number, startY: number, endX: number, endY: number) {
      ...
      const generateTypingPattern = Effect.fn("HumanBehavior.generateTypingPattern")(
      ...
      const generateScrollPattern = Effect.fn("HumanBehavior.generateScrollPattern")(
        function* (startY: number, endY: number) {
      ...
      const executeMouseMovement = Effect.fn("HumanBehavior.executeMouseMovement")(
        function* (_movement: MouseMovement) {
          return yield* Effect.logDebug("Simulating human-like mouse movement").pipe(
            Effect.as(true),
          );
        },
      );

      const executeTypingPattern = Effect.fn("HumanBehavior.executeTypingPattern")(
        function* (pattern: TypingPattern) {
          yield* Effect.forEach(pattern.delays, (delay: number) =>
            Effect.sleep(delay), {
      ...
      const executeScrollPattern = Effect.fn("HumanBehavior.executeScrollPattern")(
        function* (_pattern: ScrollPattern) {
          return yield* Effect.logDebug("Simulating human-like scroll pattern").pipe(
            Effect.as(true),
          );
        },
      );
