...
export class TokenTracker extends ServiceMap.Service<TokenTracker>()("@inspect/TokenTracker", {
  make: Effect.gen(function* () {
    ...
    const add = (tokens: number) => Effect.sync(() => { total += tokens; });
    ...
    const reset = Effect.sync(() => { total = 0; });
...
export class SensitiveDataMasker extends ServiceMap.Service<SensitiveDataMasker>()("@inspect/SensitiveDataMasker", {
  make: Effect.gen(function* () {
    const PATTERNS = [
    ...
    const mask = (text: string) => Effect.sync(() => {
      let result = text;
...
export class ToolRegistry extends ServiceMap.Service<ToolRegistry>()("@inspect/ToolRegistry", {
  make: Effect.gen(function* () {
    ...
    const register = (tool: ToolDefinition) => Effect.sync(() => {
      tools.set(tool.name, tool);
    });
    const unregister = (name: string) => Effect.sync(() => {
      tools.delete(name);
    });
    const get = (name: string) => Effect.sync(() => tools.get(name));
    const list = Effect.sync(() => [...tools.values()]);
    const listForDomain = (domain: string) => Effect.sync(() =>
      [...tools.values()].filter((t) => t.domain === domain || !t.domain)
    );
    const execute = (name: string, _input: unknown) => Effect.gen(function* () {
...
export class JudgeLLM extends ServiceMap.Service<JudgeLLM>()("@inspect/JudgeLLM", {
  make: Effect.gen(function* () {
    const evaluate = (task: string, trajectory: unknown) => Effect.gen(function* () {
...
export class NLAssert extends ServiceMap.Service<NLAssert>()("@inspect/NLAssert", {
  make: Effect.gen(function* () {
    const assert = (condition: boolean, message: string) => Effect.gen(function* () {
