...
const listProviderConfig = Config.all({
  proxies: Config.string("PROXY_LIST"),
  delimiter: Config.withDefault(Config.string("PROXY_DELIMITER"), "\n"),
});

export type ListProviderConfig = Config.Success<typeof listProviderConfig>;

const brightDataConfig = Config.all({
...
export type BrightDataConfig = Config.Success<typeof brightDataConfig>;

const oxylabsConfig = Config.all({
  username: Config.string("OXYLABS_USERNAME"),
  password: Config.string("OXYLABS_PASSWORD"),
  endpoint: Config.withDefault(
    Config.string("OXYLABS_ENDPOINT"),
    "https://oxylabs.io/api/proxy",
  ),
});

export type OxylabsConfig = Config.Success<typeof oxylabsConfig>;

const scraperAPIConfig = Config.all({
  apiKey: Config.string("SCRAPERAPI_API_KEY"),
  endpoint: Config.withDefault(
    Config.string("SCRAPERAPI_ENDPOINT"),
    "https://api.scraperapi.com",
  ),
});

export type ScraperAPIConfig = Config.Success<typeof scraperAPIConfig>;
...
export class ProxyProvider extends ServiceMap.Service<ProxyProvider>()(
  "@proxy-manager/ProxyProvider",
  {
    make: Effect.gen(function* () {
      ...
      const loadFromList = Effect.fn("ProxyProvider.loadFromList")(
        function* (config: ListProviderConfig) {
      ...
      const loadFromBrightData = Effect.fn("ProxyProvider.loadFromBrightData")(
        function* (_config: BrightDataConfig) {
      ...
      const loadFromOxylabs = Effect.fn("ProxyProvider.loadFromOxylabs")(
        function* (_config: OxylabsConfig) {
          return yield* new ProxyProviderError({
            provider: "oxylabs",
      ...
      const loadFromScraperAPI = Effect.fn("ProxyProvider.loadFromScraperAPI")(
        function* (_config: ScraperAPIConfig) {
          return yield* new ProxyProviderError({
            provider: "scraperapi",
  ...
  static layer = Layer.effect(this, this.make).pipe(
    Layer.provideMerge(ProxyPool.layer),
  );
}
