...
import { ProxyPoolExhaustedError } from "./errors.js";
...
export class ProxyRotator extends ServiceMap.Service<ProxyRotator>()(
  "@proxy-manager/ProxyRotator",
  {
    make: Effect.gen(function* () {
      ...
      let sessions: Map<string, ProxySession> = new Map();
      let currentStickyProxy: Option.Option<ProxyServer> = Option.none();
      let stickyUntil = 0;

      const assignForRequest = Effect.fn("ProxyRotator.assignForRequest")(
        function* (sessionId: string, strategy: RotationStrategy) {
          switch (strategy.mode) {
            ...
            case "per-session": {
              ...
              const proxy = yield* pool.getHealthy().pipe(
                Effect.catchTag("ProxyPoolExhaustedError", () => pool.getNext()),
              );
            ...
            case "sticky": {
              const now = Date.now();
              if (
                Option.isSome(currentStickyProxy) &&
                now < stickyUntil
              ) {
      ...
      const releaseSession = Effect.fn("ProxyRotator.releaseSession")(
      ...
      const getActiveSessions = Effect.fn("ProxyRotator.getActiveSessions")(
        function* () {
          const now = new Date();
      ...
      const cleanupExpiredSessions = Effect.fn("ProxyRotator.cleanupExpiredSessions")(
        function* () {
          const now = new Date();
