    ...
    toReturn(expected: unknown) {
      ...
      const message = () =>
        pass
          ...
          : `Expected the function to return ${String(expected)}, but got ${String(actual)}`;
      assertResult({ pass, message, actual, expected }, invert);
      return invert
        ? { pass: !pass, message, actual, expected }
