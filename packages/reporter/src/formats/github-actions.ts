...
export class GitHubActionsReporter {
  ...
  report(suite: ActionsTestSuite): void {
    ...
    for (const result of suite.results) {
      ...
      console.log(`  ${icon} ${result.name} (${result.device}) - ${result.status} [${this.fmtMs(result.duration)}]`);
      if (result.error) {
  ...
  private writeStepSummary(
    ...
    for (const result of suite.results) {
      const icon = result.status === "pass" ? "\u2705" : result.status === "fail" ? "\u274C" : "\u23ED";
      lines.push(`| ${result.name} | ${result.device} | ${icon} ${result.status} | ${this.fmtMs(result.duration)} |`);
    }
    ...
    if (failedResults.length > 0) {
      ...
      for (const result of failedResults) {
        ...
        for (const step of result.steps) {
          const si = step.status === "pass" ? "\u2705" : step.status === "fail" ? "\u274C" : "\u23ED";
    ...
    lines.push("", `*Generated by [Inspect](https://github.com/inspect) at ${new Date().toISOString()}*`);
