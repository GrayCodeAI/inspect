...
export class VisualRegression {
  ...
  async generateReport(): Promise<VisualRegressionReport> {
    ...
    const newBaselines = this.results.filter((r) => r.baselinePath === "(no baseline)").length;
    ...
    if (this.results.length > 0) {
      try {
        ...
        const filtered = this.results.filter((r) => r.baselinePath !== "(no baseline)");
        ...
        logger.debug("Slider report generation failed (optional)", {
   ...
   */
  async postToGitHub(report: VisualRegressionReport, pr: PRInfo): Promise<void> {
    const { PRComments } = await import("@inspect/git");
    ...
    if (report.failed > 0) {
      ...
      for (const r of report.results.filter((r) => !r.matched)) {
        lines.push(`| ${r.name} | ${r.mismatchPercentage.toFixed(2)}% | ${r.url ?? "-"} |`);
      }
    }
    ...
    try {
      ...
      logger.debug("GitHub status check failed (optional)", {
        err: error instanceof Error ? error.message : String(error),
      });
    }
  }
...
async function safeReadBase64(filePath: string): Promise<string> {
  try {
    ...
    logger.debug("Failed to read image for data URL", {
      filePath,
      err: error instanceof Error ? error.message : String(error),
    });
    return "";
