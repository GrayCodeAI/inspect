...
import type { CredentialProviderType, CredentialType } from "@inspect/shared";
 ...
 */
export function registerCredentialRoutes(server: APIServer, vault: CredentialVaultAPI): void {
  // POST /api/credentials
  server.post("/api/credentials", (req: APIRequest, res: APIResponse) => {
  ...
  // GET /api/credentials
  server.get("/api/credentials", (req: APIRequest, res: APIResponse) => {
    const filter: Record<string, unknown> = {};
    if (req.query.provider) filter.provider = req.query.provider;
    if (req.query.type) filter.type = req.query.type;
    if (req.query.domain) filter.domain = req.query.domain;

    const credentials = vault.list(Object.keys(filter).length > 0 ? filter : undefined);
    res.json({ credentials });
  });

  // GET /api/credentials/:id
  server.get("/api/credentials/:id", (req: APIRequest, res: APIResponse) => {
  ...
  server.put("/api/credentials/:id", (req: APIRequest, res: APIResponse) => {
    const body = req.body as Record<string, unknown> | null;
    if (!body) {
  ...
  server.delete("/api/credentials/:id", (req: APIRequest, res: APIResponse) => {
  ...
  server.post("/api/credentials/:id/test", async (req: APIRequest, res: APIResponse) => {
    try {
