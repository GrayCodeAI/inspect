...
import { PersistentTaskStore, PersistentWorkflowStore, PersistentSessionManager } from "./persistent-stores.js";
...
describe("PersistentTaskStore", () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `task-store-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it("lists all tasks", () => {
    store.set("t1", { id: "t1", status: "queued" as const, definition: { prompt: "a", url: "https://a.com", maxSteps: 10, maxIterations: 5 }, createdAt: Date.now() });
    store.set("t2", { id: "t2", status: "completed" as const, definition: { prompt: "b", url: "https://b.com", maxSteps: 10, maxIterations: 5 }, createdAt: Date.now() });
    expect(store.list()).toHaveLength(2);
  });

  it("cancels tasks", () => {
    store.set("t1", { id: "t1", status: "running" as const, definition: { prompt: "a", url: "https://a.com", maxSteps: 10, maxIterations: 5 }, createdAt: Date.now() });
    store.cancel("t1");
  ...
  it("persists across instances", async () => {
    store.set("t1", { id: "t1", status: "completed" as const, definition: { prompt: "persist", url: "https://a.com", maxSteps: 10, maxIterations: 5 }, createdAt: Date.now() });
...
describe("PersistentWorkflowStore", () => {
  ...
  it("lists workflows", () => {
    store.setWorkflow("wf-1", { id: "wf-1", name: "A", version: 1, status: "draft" as const, blocks: [], templateEngine: "handlebars" as const, strictMode: false, createdAt: Date.now(), updatedAt: Date.now() });
    store.setWorkflow("wf-2", { id: "wf-2", name: "B", version: 1, status: "active" as const, blocks: [], templateEngine: "handlebars" as const, strictMode: false, createdAt: Date.now(), updatedAt: Date.now() });
  ...
  it("executes workflows with default executor", async () => {
    store.setWorkflow("wf-1", { id: "wf-1", name: "A", version: 1, status: "active" as const, blocks: [], templateEngine: "handlebars" as const, strictMode: false, createdAt: Date.now(), updatedAt: Date.now() });
  ...
  it("stores and retrieves runs", () => {
    const run = { id: "run-1", workflowId: "wf-1", status: "completed" as const, parameters: {}, blockResults: {}, startedAt: Date.now() };
    store.setRun("run-1", run);
    expect(store.getRun("run-1")?.workflowId).toBe("wf-1");
  });

  it("lists runs filtered by workflow", () => {
    store.setRun("r1", { id: "r1", workflowId: "wf-1", status: "completed" as const, parameters: {}, blockResults: {}, startedAt: Date.now() });
    store.setRun("r2", { id: "r2", workflowId: "wf-2", status: "completed" as const, parameters: {}, blockResults: {}, startedAt: Date.now() });
    store.setRun("r3", { id: "r3", workflowId: "wf-1", status: "failed" as const, parameters: {}, blockResults: {}, startedAt: Date.now() });
  ...
  it("cancels runs", () => {
    store.setRun("r1", { id: "r1", workflowId: "wf-1", status: "running" as const, parameters: {}, blockResults: {}, startedAt: Date.now() });
    store.cancelRun("r1");
