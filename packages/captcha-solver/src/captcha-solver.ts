...
export class CaptchaSolver extends ServiceMap.Service<CaptchaSolver>()(
  "@captcha/CaptchaSolver",
  {
    make: Effect.gen(function* () {
      const config = yield* solverConfig.parse(ConfigProvider.fromEnv());

      const solveReCaptcha = Effect.fn("CaptchaSolver.solveReCaptcha")(
        function* (siteKey: string, pageUrl: string) {
      ...
      const solveHCaptcha = Effect.fn("CaptchaSolver.solveHCaptcha")(
        function* (siteKey: string, pageUrl: string) {
          return yield* solveWithProvider(config, {
            type: "hcaptcha",
            siteKey,
      ...
      const solveCloudflare = Effect.fn("CaptchaSolver.solveCloudflare")(
        function* (siteKey: string, pageUrl: string) {
          return yield* solveWithProvider(config, {
            type: "turnstile",
            siteKey,
...
const solveWithProvider = Effect.fn("CaptchaSolver.solveWithProvider")(
  function* (
    config: SolverConfig,
...
const solveVia2Captcha = Effect.fn("solveVia2Captcha")(
  function* (config: SolverConfig, params: { type: string; siteKey: string; pageUrl: string }) {
...
async function poll2CaptchaResult(
  taskId: string,
  config: SolverConfig,
): Promise<string> {
