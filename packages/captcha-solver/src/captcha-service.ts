...
export class CaptchaService extends ServiceMap.Service<CaptchaService>()(
  "@captcha/CaptchaService",
  {
    make: Effect.gen(function* () {
      ...
      const detectAndSolve = Effect.fn("CaptchaService.detectAndSolve")(function* (
        task: CaptchaTask,
      ) {
        ...
        const solution = yield* solveByType(detection.captchaType, detection.siteKey, task.pageUrl);
      ...
      const solveByType = Effect.fn("CaptchaService.solveByType")(function* (
      ...
      const verifySolution = Effect.fn("CaptchaService.verifySolution")(function* (
      ...
      const solveWithRetry = Effect.fn("CaptchaService.solveWithRetry")(function* (
        task: CaptchaTask,
      ) {
        return yield* detectAndSolve(task).pipe(Effect.retry({ times: 2 }));
      });
