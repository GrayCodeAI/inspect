...
function getDefaultRootCause(category: FailureAnalysis["category"], step: TestStep): string {
  switch (category) {
    case "element-not-found":
      return `Element "${step.target ?? step.description}" not found — may have moved, been renamed, or not yet loaded`;
    case "timeout":
      return "Operation timed out — page may be slow to load or element is behind a loading state";
    case "assertion-failed":
      return "Page state did not match expected condition after action";
    case "navigation":
      return "Page failed to navigate — URL may be broken, redirected, or blocked";
    case "network":
      return "Network request failed — API may be down, rate-limited, or returning errors";
    case "auth":
      return "Authentication required — test may need login credentials to access this page";
    case "dynamic-content":
      return "Element was removed or changed after being found — dynamic content or SPA re-render";
    default:
      return `Step failed: ${step.error ?? "unknown error"}`;
  }
}
...
function getDefaultSuggestion(category: FailureAnalysis["category"]): string {
  switch (category) {
    case "element-not-found":
      return "Try using a more resilient selector (aria-label, role, text content) or add a wait for the element";
    case "timeout":
      return "Increase timeout, add explicit wait for network idle, or check if the page requires auth";
    case "assertion-failed":
      return "Verify the expected state is correct, check if the page changed since test was written";
    case "navigation":
      return "Verify the URL is correct and accessible, check for redirects or auth requirements";
    case "network":
      return "Check if the API is running, verify network connectivity, check for rate limiting";
    case "auth":
      return "Provide authentication cookies or credentials via --cookies flag";
    case "dynamic-content":
      return "Add a wait for the element to stabilize, or use the self-healing selector strategy";
    default:
      return "Review the error message and check the page state at the time of failure";
  }
}
