...
const ALL_MODELS = GROUPS.flatMap(g => g.models.map(m => ({ ...m, provider: g.provider, color: g.color })));
...
export function ModelScreen({ onDone }: Props): React.ReactElement {
  ...
  useEffect(() => {
    const t = setTimeout(() => { phaseRef.current = "ready"; setReady(true); }, 300);
  ...
  useInput((ch, key) => {
    ...
    if (key.upArrow) { setIdx(i => (i - 1 + ALL_MODELS.length) % ALL_MODELS.length); return; }
    if (key.downArrow) { setIdx(i => (i + 1) % ALL_MODELS.length); return; }
  ...
  return (
    <Box flexDirection="column" paddingX={2} paddingY={1}>
      <Box>
        <Text color="#f97316" bold>  Select Model</Text>
          ...
          <React.Fragment key={m.id}>
            {showHeader && (
              <Box paddingX={1} marginTop={i > 0 ? 0 : 0}>
                <Text color={m.color} bold>  {m.provider}</Text>
              </Box>
            )}
            <Box paddingX={1}>
              <Text color={active ? "#f97316" : "#334155"}>
                {active ? "    ❯ " : "      "}
              </Text>
