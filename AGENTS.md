...
**Inspect** is a CLI tool that lets coding agents (Claude Code, Codex CLI, Cursor) automatically test code changes in a real browser. The workflow:

1. User runs `inspect` in their terminal
...
- **pnpm monorepo** — `inspect-cli` (Ink TUI), `@inspect/orchestrator` (orchestration), `@inspect/agent` (LLM backend), `@inspect/browser` (Playwright automation), `@inspect/cookies` (browser profile extraction), `@inspect/shared` (domain models)
- **Orchestrator** — Owns all state management, agent lifecycle, and git operations. The CLI is a stateless renderer of orchestrator state.
...
import { Cookies } from "@inspect/cookies/cookies";
import { TestPlan } from "@inspect/shared/test-plan";
