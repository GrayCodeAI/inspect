...
// TestConvertArchiveToEntries verifies that an Archive is converted
// into the expected FindingEntry records.
func TestConvertArchiveToEntries(t *testing.T) {
	now := time.Now().UTC()
	archive := &Archive{
		ScanID: "scan-123",
	...
	entries := ConvertArchiveToEntries(archive)
	if len(entries) != 3 {
		t.Fatalf("expected 3 entries, got %d", len(entries))
	}

	// Entry 0: 200 OK
	if entries[0].ScanID != "scan-123" {
		t.Errorf("entry 0: expected scan_id scan-123, got %s", entries[0].ScanID)
	}
	if entries[0].CheckName != "archive-entry" {
		t.Errorf("entry 0: expected check_name archive-entry, got %s", entries[0].CheckName)
	}
	if !entries[0].Passed {
		t.Error("entry 0: expected passed=true for status 200")
	}
	if entries[0].Severity != "info" {
		t.Errorf("entry 0: expected severity info, got %s", entries[0].Severity)
	}
	if entries[0].Message != "HTTP 200" {
		t.Errorf("entry 0: expected message 'HTTP 200', got %s", entries[0].Message)
	}
	if entries[0].Details != "<html>ok</html>" {
		t.Errorf("entry 0: unexpected details %q", entries[0].Details)
	}
	if len(entries[0].Tags) != 1 || entries[0].Tags[0] != "good" {
		t.Errorf("entry 0: unexpected tags %v", entries[0].Tags)
	}

	// Entry 1: 404
	if entries[1].Passed {
		t.Error("entry 1: expected passed=false for status 404")
	}
	if entries[1].Severity != "medium" {
		t.Errorf("entry 1: expected severity medium, got %s", entries[1].Severity)
	}

	// Entry 2: connection error
	if entries[2].Passed {
		t.Error("entry 2: expected passed=false for error entry")
	}
	if entries[2].Severity != "high" {
		t.Errorf("entry 2: expected severity high, got %s", entries[2].Severity)
	}
	if entries[2].Message != "connection refused" {
		t.Errorf("entry 2: expected message 'connection refused', got %s", entries[2].Message)
	}
}

// TestConvertArchiveToEntriesNil verifies nil archive returns nil.
func TestConvertArchiveToEntriesNil(t *testing.T) {
	entries := ConvertArchiveToEntries(nil)
	if entries != nil {
		t.Errorf("expected nil entries for nil archive, got %v", entries)
	}
}
